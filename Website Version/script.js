...
var showUrl = document.getElementById('ItemLink')
showUrl.innerHTML = "Item Link : " + link;
...
document.getElementById('dataForm').addEventListener('submit', function(event) {
    ...
    var requestUrl = `${url}?ID=${encodeURIComponent(id)}&Item%20Name=${encodeURIComponent(itemName)}&Image=${encodeURIComponent(image)}&Links=${encodeURIComponent(link)}&Price=${encodeURIComponent(price)}&Type=${encodeURIComponent(type)}`;
